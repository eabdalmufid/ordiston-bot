...
*🧪 Penggunaan :*
_${usedPrefix}${command} <Buy|sell> <item> <jumlah>_
Contoh penggunaan: _*${usedPrefix}${command} buy potion 1*_

*📮 Note :* 
bila sudah tidak ada harganya, berarti sudah tidak bisa dibeli / sudah level max

🛍️ List Barang: ${readMore}
⛊━━━┄┄┄┄┄┄┄┄┄━━━⛊
*♻ Barang   | 💲 Harga beli*
⛊━━━┄┄┄┄┄┄┄┄┄━━━⛊
*🥤 Potion:* ${potion}
                ...
                case 'foodpet':
                    ...
                    case 'rock':
                           if (global.db.data.users[m.sender].money >= Brock * count) {
                               ...
                               conn.reply(m.chat, `✔️ Sukses Membeli ${count} Rock 🪨 Dengan Harga ${Brock * count} Money 💹`, m)
                            } else conn.reply(m.chat, `Uang Anda Tidak Cukup`, m)
                        break
                    case 'umpan':
                    ...
                    case 'wood':
                            if (global.db.data.users[m.sender].money >= Bwood * count) {
                                ...
                                conn.reply(m.chat, `✔️ Sukses Membeli ${count} Wood 🪵 Dengan Harga ${Bwood * count} Money 💹`, m)
                            } else conn.reply(m.chat, `Uang Anda Tidak Cukup`, m)
                        break
                     case 'aqua':
                ...
                switch (_type) {
                    ...
                    case 'rock':
                        if (global.db.data.users[m.sender].rock >= count * 1) {
                            ...
                            global.db.data.users[m.sender].rock -= count * 1
                            conn.reply(m.chat, `✔️ Sukses Menjual ${count} Rock 🪨 Dengan Harga ${Srock * count} Money 💹`.trim(), m)
                        } else conn.reply(m.chat, `🪨 Rock Kamu Tidak Cukup`.trim(), m)
                        break
                    case 'wood':
                        if (global.db.data.users[m.sender].wood >= count * 1) {
                            global.db.data.users[m.sender].money += Swood * count
                            global.db.data.users[m.sender].wood -= count * 1
                            conn.reply(m.chat, `✔️ Sukses Menjual ${count} Wood 🪵 Dengan Harga ${Swood * count} Money 💹`.trim(), m)
                        } else conn.reply(m.chat, `🪵 Wood Kamu Tidak Cukup`.trim(), m)
                        break
                    case 'string':
