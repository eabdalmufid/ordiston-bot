...
import handlingUserExistsOnGroup from "./lib/isExistOnGroup.js";
    ...
    try {
        ...
        try {
            ...
            if (settings) {
                ...
                if (!("validatingUserOnGroup" in settings)) settings["validatingUserOnGroup"] = {
				    enable: false,
					message: "Please join our group in order to use all feature on Bot",
					groupJid: null,
				}
        ...
        const isPrems = isROwner || global.prems.map(v => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender) || db.data.users[m.sender].premiumTime > 0
        if (!isOwner && !m.fromMe && opts['self']) return; // Fix self?
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || "")[0])) {
                ...
                if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                    let chat = global.db.data.chats[m.chat]
                    let user = global.db.data.users[m.sender]
                    if (name != "owner-unbanchat.js" && name != "owner-exec.js" && name != "owner-exec2.js" && name != "tools-delete.js" && name != "enable.js" && chat?.isBanned)
                        return // Except this
                ...
                if (xp > 200)
                this.sendMessage(m.chat, { text: `[❗] *Sepertinya Anda Bermain Curang, Menggunakan Calculator*`, mentions: [m.sender] }, { quoted: m, ephemeralExpiration: ephemeral })
                else
                    m.exp += xp
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.sendMessage(m.chat, { text: `[❗] *Limit Anda Habis, Beberapa Command Tidak Bisa Di Akses*`, mentions: [m.sender] }, { quoted: m, ephemeralExpiration: ephemeral })
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.sendMessage(m.chat, { text: `[💬] Diperlukan level *${plugin.level}* untuk menggunakan perintah ini. Level kamu *${_user.level}🎋*\n*${plugin.level}* level is required to use this command. Your level is *${_user.level}🎋*`, mentions: [m.sender] }, { quoted: m, ephemeralExpiration: ephemeral })
                    continue // If the level has not been reached
                }
                if (!m.isGroup && noPrefix && db.data.settings[this.user.jid]["validatingUserOnGroup"]?.["enable"]) {
                    const isUserExist = await handlingUserExistsOnGroup(m, {
                        conn: this,
                        groupJid: db.data.settings[this.user.jid]?.["validatingUserOnGroup"]["groupJid"],
                    })
                    if (!isUserExist) { return }
                    else if (!(isOwner || isPrems)) {
                    	return this.reply(m.chat, "Harap upgrade ke premium untuk menggunakan bot di private chat", m)
                    }
                }
                let extra = {
                    match,
                ...
                try {
                    if (!isOwner) {
                        if (m.isGroup && (new Date - global.db.data.chats[m.chat].delay < 3000)) return m.reply('Jangan spam kak, tunggu *3 detik*')
                        if (!m.isGroup && (new Date - global.db.data.users[m.sender].delay < 3000)) return m.reply('Jangan spam kak, tunggu *3 detik*')
                    }
        ...
        case "remove":
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://raw.githubusercontent.com/abclimadasar/silence/master/src/avatar_contact.png'
                    try {
                        ...
                        await conn.sendFthumb(id, `Ordiston Bot`, welcomeText.replace("@user", "@" + participants[0].split("@")[0]), pp, sgc, fliveLocc, { mentions: [participants[0]] })
